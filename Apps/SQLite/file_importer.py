import sqlite3, csv, json, xmltodict
...
class Importer:
    ...
    def import_xml(self, filename):
        with open(filename + '.xml', 'r', encoding='utf-8') as file:
            my_xml = file.read()
            d = xmltodict.parse(my_xml)
            self.dictionary_to_sql(filename, d['dataset']['record'])

    def dictionary_to_sql(self, filename, d: dict):
        ...
        for row in d:
            for title in row:
                item = row[title]
                #print("Title: " + title + ", Item: " + str(item))
                insertHeader += title + ", "
